import pytest
from app.main import app
import json

@pytest.fixture
def client():
    app.config['TESTING'] = True
    with app.test_client() as client:
        yield client

def test_health_check(client):
    response = client.get('/')
    assert response.status_code == 200
    data = response.get_json()
    assert data['status'] == 'healthy'
    assert data['service'] == 'URL Shortener API'

def test_shorten_url(client):
    response = client.post('/api/shorten', json={"url": "https://example.com"})
    assert response.status_code == 201
    data = response.get_json()
    assert "short_code" in data
    assert "short_url" in data

def test_redirect(client):
    # First shorten the URL
    post_res = client.post('/api/shorten', json={"url": "https://example.com"})
    short_code = post_res.get_json()["short_code"]

    # Now test redirect
    redirect_res = client.get(f"/{short_code}", follow_redirects=False)
    assert redirect_res.status_code == 302  # Found
    assert "https://example.com" in redirect_res.headers["Location"]

def test_stats(client):
    # Shorten URL
    post_res = client.post('/api/shorten', json={"url": "https://example.com"})
    short_code = post_res.get_json()["short_code"]

    # Trigger a redirect to generate click
    client.get(f"/{short_code}")

    # Now check stats
    stats_res = client.get(f"/api/stats/{short_code}")
    assert stats_res.status_code == 200
    data = stats_res.get_json()
    assert data["url"] == "https://example.com"
    assert data["clicks"] == 1
    assert "created_at" in data

def test_invalid_url(client):
    res = client.post('/api/shorten', json={"url": "not-a-valid-url"})
    assert res.status_code == 400
    data = res.get_json()
    assert "Invalid URL format" in data["error"]

def test_invalid_short_code(client):
    res = client.get("/api/stats/invalid123")
    assert res.status_code == 404
    data = res.get_json()
    assert "Short code not found" in data["error"]
